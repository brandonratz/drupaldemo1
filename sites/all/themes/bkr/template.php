...
// Add Zen Tabs styles
if (theme_get_setting('bkr_tabs')) {
  drupal_add_css( drupal_get_path('theme', 'bkr') .'/css/tabs.css');
}

function bkr_preprocess_html(&$vars) {
  global $user;

  // Add role name classes (to allow css based show for admin/hidden from user)
  foreach ($user->roles as $role){
    $vars['classes_array'][] = 'role-' . bkr_id_safe($role);
  }
  ...
  if (!$vars['is_front']) {
    ...
    $vars['classes_array'][] = bkr_id_safe('page-'. $path);
    $vars['classes_array'][] = bkr_id_safe('section-'. $section);
  ...
  // Add IE classes.
  if (theme_get_setting('bkr_ie_enabled')) {
    $bkr_ie_enabled_versions = theme_get_setting('bkr_ie_enabled_versions');
    if (in_array('ie8', $bkr_ie_enabled_versions, TRUE)) {
...
function bkr_preprocess_page(&$vars, $hook) {
...
function bkr_preprocess_node(&$vars) {
  ...
  // Merge first/last class (from bkr_preprocess_page) into classes array of current node object.
  $node = $vars['node'];
...
function bkr_preprocess_block(&$vars, $hook) {
  // Add a striping class.
 ...
 */
function bkr_breadcrumb($variables) {
  $breadcrumb = $variables['breadcrumb'];
  // Determine if we are to display the breadcrumb.
  $show_breadcrumb = theme_get_setting('bkr_breadcrumb');
  if ($show_breadcrumb == 'yes' || $show_breadcrumb == 'admin' && arg(0) == 'admin') {

    // Optionally get rid of the homepage link.
    $show_breadcrumb_home = theme_get_setting('bkr_breadcrumb_home');
    ...
    if (!empty($breadcrumb)) {
      $breadcrumb_separator = theme_get_setting('bkr_breadcrumb_separator');
      $trailing_separator = $title = '';
      if (theme_get_setting('bkr_breadcrumb_title')) {
        $item = menu_get_item();
      ...
      elseif (theme_get_setting('bkr_breadcrumb_trailing')) {
        $trailing_separator = $breadcrumb_separator;
      }
 ...
 *
 */
function bkr_menu_link(array $variables) {
  ...
  $element['#attributes']['class'][] = bkr_id_safe($element['#title']);
 ...
 */
function bkr_preprocess_menu_local_task(&$variables) {
 ...
 */
function bkr_menu_local_tasks(&$variables) {
  $output = '';
